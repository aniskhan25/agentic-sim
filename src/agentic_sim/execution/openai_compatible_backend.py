...
class OpenAICompatibleExecutionBackend:
    """Shared logic for any OpenAI-compatible chat-completions execution
    backend -- request building, retry, JSON parsing/repair, role_policy
    wiring, and receipt/result construction. Extracted from
    AittaExecutionBackend (item 19): every piece here was already fully
    generic, with zero Aitta-specific assumptions baked in -- confirmed by
    direct line-by-line reading before extraction, not assumed. Concrete
    subclasses (AittaExecutionBackend, SelfHostedExecutionBackend) own
    their own env-var resolution, required-ness checks, `name`, and
    `capabilities` -- the two backends genuinely disagree on capability
    defaults (e.g. supports_prefix_caching), so there is no single sane
    default here.
    """
    ...
    def _send(self, payload: dict[str, Any]) -> tuple[dict[str, Any], int]:
        ...
        raise RuntimeError(
            f"{self.name} request failed after {self.max_retries + 1} attempt(s): {last_error}"
        ) from last_error
    ...
    def _result_from_proposal(
        ...
        manifest = self.platform_manifest
        receipt = ExecutionReceipt(
            ...
            accelerator=manifest.accelerator if manifest else None,
            host_architecture=manifest.host_architecture if manifest else None,
            serving_runtime=manifest.serving_runtime if manifest else None,
            manifest_mode=manifest.manifest_mode if manifest else None,
        )
        metadata["execution_receipt"] = to_jsonable(receipt)
        if manifest is not None:
            metadata["platform_manifest"] = to_jsonable(manifest)
    ...
    for candidate in _iter_json_candidates(cleaned):
        ...
        for attempt in (candidate, _strip_trailing_commas(candidate)):
            ...
            if isinstance(data, dict):
                return data
            last_exc = ValueError("model output must be a JSON object")
