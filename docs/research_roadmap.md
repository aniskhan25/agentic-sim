...
6. Complete provider-neutral contract schemas and per-atom provenance for messages, environment actions, and state mutations.
7. Align empty-commit autonomy reporting with the target definition and retain a separately labeled message/action-only compatibility metric if needed.
8. Add causal parents, state versions, and the causal verifier.
9. Build the seeded minimum DAG kernel and component-level measurement harness.
10. Implement the atomic state-store commit conformance contract.
11. Define the asynchronous provider conformance interface.
12. Add sequential, naive-concurrent, barrier, and causal-only baselines.
13. Implement the capability, queue-awareness, backpressure, and prefix-grouping ablation ladder.
