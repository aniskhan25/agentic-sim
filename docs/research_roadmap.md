...
**Real, honest result — the two systems disagree**: LUMI (single MI250X GCD): `full` is **10.3% slower** than `sequential` (useful-agent-steps/sec: 0.249 vs. 0.223), and the mean±1-stdev bands do **not** overlap — a real, statistically distinguishable regression, not noise. Roihu (single GH200): `full` is **12.2% faster** than `sequential` (0.814 vs. 0.913), but the bands **do** overlap — with only 3 repetitions this is not statistically distinguishable from noise, just directionally suggestive. Zero excluded repetitions on either system (both real 3-repeat runs completed cleanly). This is a genuinely informative pilot outcome, not a failure: it's directly relevant to H4 ("no policy is optimal across... workload characteristics") and H7 ("scheduler effects generalize across accelerator ecosystems, although magnitude and responsible features may differ") — with only 3 reps and one workload/placement combination, it cannot confirm or refute either hypothesis, but it demonstrates the full measurement pipeline (real warm-up, real repeated dispatch-policy comparison, real exclusion handling, real cross-system reporting) works end to end for the first time. Raw results committed as real artifacts, matching item 14's precedent: `docs/baseline/b1_pilot_lumi_result.json`, `docs/baseline/b1_pilot_roihu_result.json`. **Still fully open**: the actual 10-repetition primary study, both placement levels, all five workload families, all 7 policy rungs, and B2 mode — this pilot covered 1 of 5 workloads × 1 of 2 placements × 2 of 7 policies × 3 of 10 reps, on B1 only.
