...
**Full-node placement, first real attempt, both systems**: asked to tackle full-node next, both deployment manifests already specify the right strategy for a dense 7B model — independent replicas per GPU (LUMI: 8 MI250X GCDs; Roihu: 4 GH200s), not tensor-parallelism (reserved for a future 70B stretch case) — so no new backend/dispatch-policy code was needed, only new job orchestration: launch N independent `vllm serve` processes (one per GPU, `CUDA_VISIBLE_DEVICES`/`ROCR_VISIBLE_DEVICES` pinning), health-check all N, then run N concurrent, independent copies of the unmodified `scripts/run_b1_pilot.py` (one per replica) — `storm` only, all 7 policies, 3 reps (a bounded first attempt, mirroring the very first single-device pilot's scale, not the 10-rep confirmatory scale). "Delivered node capacity" = sum of each policy's per-replica useful-agent-steps/sec means, per `evaluation_plan.md`'s own framing.

Roihu's first attempt (4 replicas) succeeded cleanly on the first try: zero exclusions, and every replica's per-policy numbers land right inside the already-established single-device range (e.g. `causal_only` per-replica 1.30-1.57 vs. single-device's ~1.3-1.5) — **no meaningful host contention**, four GH200s deliver close to 4× single-GPU throughput concurrently. LUMI's first attempt failed (exit 1, aborted after 6m12s): 7 of 8 replicas came up in seconds, but replica 0 — the first to read the ~15 GB model weights from network storage — was still mid-load at the 5-minute health-check cutoff; replicas 1-7 loaded fast only because replica 0's read had already warmed the OS page cache. A real, useful operational finding for any future full-node LUMI deployment: **the first cold model load is much slower than subsequent warm-cache loads**, and a health-check timeout sized for a single warm load is too short for the first one. Fixed by widening the per-replica timeout to 15 minutes and re-running: all 8 replicas came up healthy, zero exclusions, and per-replica numbers again matched the single-device baseline closely (`causal_only` per-replica avg 0.369 vs. single-device's ~0.37-0.40) — no contention once past the cold-load hurdle.

**Delivered full-node capacity** (summed across replicas):

| policy | Roihu (4 GH200s) | LUMI (8 MI250X GCDs) |
|---|---|---|
| sequential | 3.32 | 2.01 |
...
**Real aggregation/reporting tooling built**: `evaluation_plan.md`'s Required Outputs list calls for "aggregation and plotting scripts" and "machine-readable result tables" — every result up to this point had only ever been reported via one-off chat analysis, never committed or re-runnable. New `observability/b1_results_summary.py::summarize_b1_results` (tested, `tests/test_b1_results_summary.py`) and `scripts/aggregate_b1_results.py` (an explicit, hand-written manifest of every real result file — not filename inference, since the JSON payloads carry no system/workload/placement metadata of their own) regenerate `docs/baseline/b1_summary.{csv,md}`, `docs/baseline/b1_contrasts_summary.{csv,md}`, and `docs/baseline/b1_retune_sweep_summary.{csv,md}` from the 32 real files already collected. Every regenerated number matches what was already reported here exactly (spot-checked), confirming the tool reproduces real results, not new ones. `capacity_type` honestly distinguishes `per_device` rows (passed through directly) from `node_total` rows (full-node replicas summed, with **properly propagated stdev** — `Var(sum) = sum(Var(x_i))` — rather than a fabricated or omitted error bar).

Building this surfaced a real refinement: with proper stdev propagation instead of eyeballed point estimates, **LUMI's `supply_chain` full-node `full_vs_causal_only` is `bands_overlap: False`** (-10.7%, a real, statistically distinguishable regression) — unlike the other three full-node `full_vs_causal_only` comparisons, which remain noise-level uncertain at this 3-rep/replica scale. `docs/b1_results_tables.md` documents how to regenerate the tables and read `capacity_type`/`source`, closing the "documented commands for each paper table" requirement. Plotting/figures remain explicitly out of scope — a separate, later step once these tables exist to plot from.
...
**B2 (platform-tuned) sweep, first run, both systems**: `docs/hpc_data_collection_procedures.md`'s Platform-tuned mode section had a frozen *selection procedure* (primary metric: useful agent-steps/sec; hard disqualifier: any KV-cache preemption; tie-break: overlapping bands → simpler/lower-resource wins) but no configuration values had ever been selected for either system. New `src/agentic_sim/observability/b2_selection.py::select_best_candidate` (tested, `tests/test_b2_selection.py`) implements that rule exactly, and a new `--policies` flag on `scripts/run_b1_pilot.py` lets the sweep hold `causal_only` fixed while varying serving parameters. Given the full ~192-config factorial across 5 dimensions (`max_num_batched_tokens`, `max_num_seqs`, `gpu_memory_utilization`, attention backend, `kv_cache_dtype`) is infeasible (each config needs a real `vllm serve` restart), the user chose one-factor-at-a-time (OFAT/coordinate descent) instead — 15 configs per system, 3 reps each, `storm` workload, single-device placement — explicitly noted as possibly missing cross-dimension interaction effects a full grid would catch.

The first LUMI attempt hit two real bugs, both fixed and re-confirmed (see `docs/b2_frozen_configuration.md`'s Operational findings section for the full detail): a scripting bug embedding bash's `true`/`false` into a Python snippet crashed every candidate and silently cascaded a blank winner through the rest of the run to a full 2h30m timeout, and a genuine pre-existing robustness gap in `storm_env.py`/`supply_chain_env.py` (`apply_actions` indexing `action.payload["region"]`/`["delta"]` with no default, unlike the `.get(...)` pattern already used one line above) let a real vLLM completion's occasionally-missing field crash pilot warm-up outright rather than just excluding one repetition — this is what actually killed the `kv_cache_dtype=default` candidate specifically, not a real finding about that parameter. Fixed (commit `dbd2a88`, new tests in `tests/test_storm_scenario.py`/`tests/test_supply_chain_scenario.py`): a malformed action is now a no-op. Confirmed no other candidate in the corrupted run had any silently-excluded repetitions, so only `kv_cache_dtype` needed re-measurement, done via a small standalone re-run holding the other four dimensions at their already-confirmed winners. Roihu's sweep (run after the same fixes were applied to its job script) completed cleanly end to end on the first attempt, zero crashes.

**Selected B2 configuration, both systems** (full divergence table and per-dimension real-vs-tie-break honesty in `docs/b2_frozen_configuration.md`):

| Parameter | B1 | B2 LUMI | B2 Roihu |
|---|---|---|---|
| `max_num_batched_tokens` | 16384 | 8192 | 8192 |
| `max_num_seqs` | 64 | 32 | 32 |
| `gpu_memory_utilization` | 0.90 | 0.90 | 0.85 |
| Attention backend | baseline | baseline | baseline |
| `kv_cache_dtype` | fp8_e4m3 | fp8_e4m3 | fp8_e4m3 |

Read plainly: 3 of 5 dimensions (batched tokens, num_seqs, gpu_memory_utilization) resolved via the frozen tie-break rule, not a clear measured win — at 3 reps/candidate, real differences in these dimensions are within noise for this workload/model/hardware. Only `kv_cache_dtype` shows a real, non-overlapping win on both systems (LUMI: 0.413 vs. 0.217; Roihu: 1.778 vs. 0.850). One genuine finding beyond the frozen config itself: **both platform-specific attention-backend alternatives are confirmed available and functional** — AITER (LUMI) and FLASHINFER (Roihu) both start and serve real traffic without error — but neither beats baseline; FLASHINFER is in fact a real, non-overlapping *loss* on Roihu (1.242 vs. baseline's 1.525). Raw data: `docs/baseline/b2_sweep_{lumi,roihu}_trace.json` (every candidate, not just winners), `docs/baseline/b2_frozen_config_{lumi,roihu}.json`, `docs/baseline/b2_sweep_lumi_kvcache_rerun.json` (the corrected re-measurement). **Still fully open**: a higher-repetition confirmatory B2 sweep to distinguish real effects from tie-break noise; B2's actual primary 10-repetition data collection using this frozen config; `supply_chain`'s and full-node placement's own B2 configuration (this sweep covered `storm`, single-device only, matching B1's own phasing); and everything else already logged as open above.

**B1-vs-B2 confirmatory comparison, both systems**: with both configs frozen, the natural next question — does platform-tuning actually help — was tested for real: `causal_only` policy held fixed (isolating the serving-config effect from the already-settled policy-ladder question), `storm` + `supply_chain`, single-device, **10 repetitions** (B1's frozen confirmatory scale, not B2 sweep's 3-rep exploratory scale), both systems, both configs — 8 real runs, `scripts/aggregate_b1_vs_b2_results.py` (new, reuses `b1_pilot.py::_relative_contrast` unmodified) computing the contrast. One LUMI cycle (`b2`/`supply_chain`) was cut off by the job's 1-hour time limit after the other three cycles took longer than expected on LUMI's shared partition (no bug — zero exclusions in any completed cycle, just real scheduling variance); a small standalone re-run of just that one cycle completed cleanly and supplied the missing data point.

**Result: a real, honest, and somewhat surprising finding — B2 is not statistically distinguishable from B1 in any of the 4 (system, workload) combinations at this scale:**

| system | workload | B1 mean | B2 mean | relative improvement | bands overlap |
|---|---|---|---|---|---|
| LUMI | storm | 0.362 | 0.379 | +4.6% | **yes** |
| LUMI | supply_chain | 0.320 | 0.511 | +59.7% | **yes** (B1's stdev is large here, 0.183 vs. a 0.320 mean) |
| Roihu | storm | 1.553 | 1.451 | -6.6% | **yes** |
| Roihu | supply_chain | 1.722 | 1.909 | +10.9% | **yes** |

Despite `kv_cache_dtype` alone showing a large, clear, non-overlapping win in the isolated B2 selection sweep (LUMI: +90%; Roihu: +109%), comparing B1's and B2's **full** configs against each other — all 5 parameters together, including the batching-parameter changes (`max_num_batched_tokens`/`max_num_seqs`) that the selection sweep itself only resolved via tie-break, not a clear measured win — shows no statistically significant net effect on either system for either workload at 10 reps. This is a legitimate, informative outcome, not a failure of the sweep: it's consistent with the selection sweep's own honest caveat (3 of 5 dimensions were tie-break picks, not proven wins) and suggests that whatever real gain `kv_cache_dtype` contributes in isolation is not large enough to survive combined with the other changes and real cross-run noise at this repetition count. Raw data: `docs/baseline/b1_vs_b2_{lumi,roihu}_{b1,b2}_{storm,supply_chain}_result.json` (8 files), `docs/baseline/b1_vs_b2_comparison.{csv,md}`.

**Confirmed at 30 reps: this is a real null result, not under-powered noise.** The 10-rep point estimates ranged widely (-6.6% to +59.7%), with LUMI's `supply_chain` B1 measurement carrying a suspiciously large stdev (0.183 on a 0.320 mean) — worth checking whether more repetitions would resolve a real effect hiding in that noise, so the same 8 runs were repeated at 30 reps each (`scripts/aggregate_b1_vs_b2_results.py` extended with a second manifest, reusing the exact same job mechanism; one submission needed no changes beyond `--repeats 30` and a new output filename per run). Result: **every one of the 4 comparisons converges to a small, non-significant difference** (LUMI storm: +1.5%; LUMI supply_chain: +0.1%; Roihu storm: +1.2%; Roihu supply_chain: +1.1% — all bands overlap). The earlier LUMI `supply_chain` outlier fully resolved: B1's 30-rep mean (0.551) is nearly identical to B2's (0.552), confirming the 10-rep run's 0.320 B1 mean was simply an unlucky noisy sample, not a real B1 weakness. **This settles the question for this scope**: B2's platform-tuned config, as selected, delivers no measurable throughput advantage over B1's common-denominator config for `causal_only` on `storm`/`supply_chain` at single-device placement, on either system. Raw data: `docs/baseline/b1_vs_b2_{lumi,roihu}_{b1,b2}_{storm,supply_chain}_30rep_result.json` (8 files), `docs/baseline/b1_vs_b2_comparison_30rep.{csv,md}`.

**The null result itself was explained, then reversed, once real concurrent load was actually created.** Investigating *why* B1 and B2 never differ (a research agent plus direct code reading, not more repetitions) found the true cause: B1 and B2 share the identical `--kv-cache-dtype fp8_e4m3` (the one dimension with a real measured effect in the OFAT sweep contributes nothing to a B1-vs-B2 diff), the three dimensions that *do* differ were themselves tie-break picks (never proven wins), and -- the deciding factor -- **the experiment never generated enough real concurrent backend load to stress either config's `--max-num-seqs` cap at all**. Three independent, compounding, previously-unexamined constraints, found by tracing the actual call path: `scripts/run_b1_pilot.py` never set `agent_replicas` (`storm`/`supply_chain` ran with only 4-5 agents), `SimulationEngine.step` caps event processing at `max_events_per_tick=32` regardless of roster size (`engine/simulation_engine.py:68`), and `CausalOnlyDispatchPolicy`'s own `ThreadPoolExecutor` defaults to `max_workers=8` (`scheduling/dispatch_policy.py:120-124`) with no override in `run_b1_pilot.py` -- confirmed, by tracing `submit()`→`run_batch([request])`'s single-request branch, that `--self-hosted-max-concurrency` never even applies to `causal_only`'s real code path. Net effect: at most 8 concurrent requests ever reached vLLM, regardless of how the configs' 32/64 caps were set.

Three new flags on `scripts/run_b1_pilot.py` (`--agent-replicas`, `--max-events-per-tick`, `--dispatch-max-workers`) fix all three constraints, reusing scenario factories' and `CausalOnlyDispatchPolicy`'s already-existing constructor parameters -- no new mechanism, just finally wiring up what already existed. An exploratory pilot (5 reps, `agent_replicas` sized to land ~58-78 real concurrent requests/tick -- confirmed via `backend_steps`, comfortably past B2's 32 and close to/past B1's 64) reran the same 8 cycles on both systems.

**Result: a real, mechanistically-explained reversal.** B2 is now measurably *slower* than B1 under genuine load, on every one of the 4 (system, workload) combinations, non-overlapping (real) on 2 of 4:

| system | workload | B1 mean | B2 mean | B2 vs. B1 | bands overlap | B1 latency | B2 latency |
|---|---|---|---|---|---|---|---|
| LUMI | storm | 4.538 | 3.895 | -14.2% | overlap (close) | 12.45s | 18.61s |
| LUMI | supply_chain | 4.054 | 3.186 | **-21.4%** | **no overlap** | 12.37s | 17.22s |
| Roihu | storm | 17.954 | 14.848 | -17.3% | overlap (close) | 2.97s | 4.00s |
| Roihu | supply_chain | 15.548 | 13.208 | **-15.1%** | **no overlap** | 2.50s | 4.24s |

Latency data confirms the mechanism directly: B2's mean per-request latency is 35-70% higher than B1's on every combination -- consistent with more real queueing against a 32-request cap than a 64-request one, not a fluke. Zero KV-cache preemption on either system for any of the 8 cycles (checked directly in the server logs, not just the automated grep) -- this is a real batching/queueing efficiency effect, not the hard disqualifier. This means the B2 selection sweep's own tie-break choices (which favored smaller/simpler values for `max-num-batched-tokens`/`max-num-seqs`/`gpu-memory-utilization` in the absence of a *proven* difference) turn out to have picked the genuinely worse option once tested under load the sweep itself never created either -- an important, honest correction to `docs/b2_frozen_configuration.md`'s selected config, not just a footnote. Raw data: `docs/baseline/b1_vs_b2_highconcurrency_{lumi,roihu}_{b1,b2}_{storm,supply_chain}_result.json` (8 files), `docs/baseline/b1_vs_b2_comparison_highconcurrency.{csv,md}`.

**Both borderline (overlapping-band) storm cases confirmed at 15 reps.** LUMI's and Roihu's `storm` combinations were the two with overlapping bands above (-14.2% and -17.3% at 5 reps, respectively) -- everything else already resolved non-overlapping at that scale. Reran both, `causal_only`/`storm` only, same high-concurrency settings (`--agent-replicas 48 --max-events-per-tick 128 --dispatch-max-workers 128`), 15 repetitions instead of 5:

| system | workload | B1 mean | B2 mean | B2 vs. B1 | bands overlap |
|---|---|---|---|---|---|
| LUMI | storm | 4.642 | 3.796 | **-18.2%** | **no overlap** |
| Roihu | storm | 19.008 | 14.821 | **-22.0%** | **no overlap** |

Both resolve cleanly to real, non-overlapping effects, in the same direction and similar magnitude to the 5-rep exploratory estimates -- confirming the high-concurrency reversal was a real effect the whole time, just under-powered at n=5 for these two combinations specifically. Zero KV-cache preemption on either system (checked directly in both server logs). **All 4 (system, workload) combinations are now confirmed non-overlapping**: B2 is measurably, consistently slower than B1 under genuine concurrent load on every combination tested. Raw data: `docs/baseline/b1_vs_b2_highconcurrency_{lumi,roihu}_{b1,b2}_storm_15rep_result.json` (4 files), `docs/baseline/b1_vs_b2_comparison_highconcurrency_storm_15rep.{csv,md}`.

**B2's tie-break dimensions re-swept under real concurrent load -- `max_num_seqs` corrected on both systems.** The B2 selection sweep's own honesty caveat (3 of 5 dimensions were tie-break picks, not proven wins) plus the mechanism just confirmed above (the sweep's measurements went through the same unfixed `run_b1_pilot.py`, so no candidate's `max_num_seqs` was ever actually stressed) left an open, actionable question: would `max_num_batched_tokens`/`max_num_seqs`/`gpu_memory_utilization`'s picks resolve differently under real load? Re-swept those 3 dimensions only (`kv_cache_dtype` and attention backend already show real, load-independent evidence) with the same `--agent-replicas 48 --max-events-per-tick 128 --dispatch-max-workers 128` settings, `storm`-only, 3 reps/candidate (matching the original sweep's own scale for direct comparability) -- 11 real `vllm serve` cycles per system, zero preemption, zero failed candidates, LUMI ran 1h49m, Roihu ran 32m.

**Result**: `max_num_batched_tokens` re-confirms unchanged (8192, still fully overlapping bands on both systems). But `max_num_seqs` is a real, non-overlapping correction on **both systems** -- the original tie-break pick of 32 turns out to be a genuine loser under real load, not a noise artifact:

| system | dimension | old winner | new winner | evidence |
|---|---|---|---|---|
| LUMI | `max_num_seqs` | 32 (tie-break) | **64** | 32 (3.80±0.16) no longer overlaps the best candidate's band (128: 4.96±0.43) -- real exclusion |
| LUMI | `gpu_memory_utilization` | 0.90 (tie-break; 0.85 was a real loser at low concurrency) | **0.85** | all 3 now fully tied (3.72-3.74) -- 0.85 wins the tie-break, superseding 0.90 |
| Roihu | `max_num_seqs` | 32 (tie-break) | **128** | 32 (14.24±0.75) and 64 (20.13±0.32) both fall outside the best candidate's band (256: 21.78±1.14) -- real exclusion |
| Roihu | `gpu_memory_utilization` | 0.85 (tie-break) | 0.85 (re-confirmed) | all 3 remain fully tied (14.16-14.63) |

The corrected `max_num_seqs` values are notably closer to (LUMI: 64, matching B1 exactly) or larger than (Roihu: 128, exceeding B1's 64) B1's own value -- consistent with the B1-vs-B2 finding above that B2's smaller `max_num_seqs=32` cap was the mechanistic cause of its slowdown under load. This closes the "should B2 be re-tuned using a concurrency-aware methodology" open item raised above: yes, and the correction is now applied -- `docs/b2_frozen_configuration.md` and `docs/baseline/b2_frozen_config_{lumi,roihu}.json` updated accordingly. Raw data: `docs/baseline/b2_concurrent_sweep_{lumi,roihu}_trace.json`.

**B1-vs-B2 rerun with the corrected config: the earlier "B2 is slower" finding does not survive the fix.** Since B1's config never changed, only B2 needed new real measurements -- reran B2/storm (15 reps) and B2/supply_chain (5 reps) with the corrected `max_num_seqs`/`gpu_memory_utilization` under the same real-load settings (`--agent-replicas 48/36 --max-events-per-tick 128 --dispatch-max-workers 128`), compared against the existing, unchanged B1 result files at matching rep counts. Zero preemption on either system, both cycles clean on both systems (LUMI 36 minutes, Roihu ~10 minutes real GPU time):

| system | workload | B1 mean | B2 (corrected) mean | B2 vs. B1 | bands overlap |
|---|---|---|---|---|---|
| LUMI | storm | 4.642 | 4.951 | +6.7% | overlap |
| LUMI | supply_chain | 4.054 | 4.257 | +5.0% | overlap |
| Roihu | storm | 19.008 | 21.885 | **+15.1%** | **no overlap** |
| Roihu | supply_chain | 15.548 | 18.517 | **+19.1%** | **no overlap** |

**The result fully reverses.** LUMI's B2 is no longer a real loss -- it's now statistically indistinguishable from B1 (both bands overlap), a modest positive point estimate rather than the earlier confirmed -18.2%/-21.4% real losses. Roihu's B2 goes further: it now **measurably beats** B1, non-overlapping on both workloads, versus the earlier confirmed -22.0%/-15.1% real losses. This closes the loop the whole investigation opened: the original "B2 delivers no measurable advantage" (10/30-rep) finding was an artifact of insufficient concurrency; the "B2 is slower under real load" finding was real but itself an artifact of an under-tested `max_num_seqs` pick; with both bugs fixed, B2's platform-tuned config is at parity with B1 on LUMI and a real, measurable win on Roihu. Raw data: `docs/baseline/b1_vs_b2_highconcurrency_{lumi,roihu}_b2_{storm,supply_chain}_corrected_result.json`, `docs/baseline/b1_vs_b2_comparison_corrected.{csv,md}`.

**LUMI's overlapping-band edge confirmed as noise, not a real effect.** Both B1 and B2 rerun on LUMI only (Roihu already resolved non-overlapping, no ambiguity to confirm) at tripled reps -- storm 15->30, supply_chain 5->15, matching the same tripling discipline already used to resolve the earlier 10-vs-30-rep and 5-vs-15-rep borderline cases above. All 4 cycles clean, zero preemption, 2h21m real GPU time:

| workload | reps | B1 mean | B2 (corrected) mean | B2 vs. B1 | bands overlap |
|---|---|---|---|---|---|
| storm | 30 | 4.656 | 4.729 | +1.6% (was +6.7% at n=15) | overlap |
| supply_chain | 15 | 4.055 | 4.080 | +0.6% (was +5.0% at n=5) | overlap |

The point estimate shrinks toward zero as n increases on both workloads -- the opposite of what a real, under-powered effect would do (which would hold steady or tighten around a nonzero value). This is genuine convergence toward parity, not noise obscuring a real edge: on LUMI, the corrected B2 config and B1 are statistically indistinguishable, full stop. Combined with Roihu's confirmed non-overlapping win, the final, fully-confirmed picture for `causal_only`/single-device is: **B2's corrected platform-tuned config is a real, measurable win on Roihu and a genuine tie on LUMI** -- never worse than B1 on either system, which is itself the intended outcome of platform-specific tuning. Raw data: `docs/baseline/b1_vs_b2_highconcurrency_lumi_{b1,b2}_storm_30rep(_corrected)?_result.json`, `docs/baseline/b1_vs_b2_highconcurrency_lumi_{b1,b2}_supply_chain_15rep(_corrected)?_result.json`, `docs/baseline/b1_vs_b2_comparison_corrected_lumi_confirm.{csv,md}`.

**The 7-rung scheduler ladder itself was confounded by the same class of bug -- rerun under real concurrent load.** The scheduler decision gate (item 14) and its real-hardware retune/reconfirm (this item, above) are the evidence base for the paper's *primary* contribution claim (`full` beats `causal_only`). That evidence was collected entirely at `agent_replicas=1` -- the exact regime just proven unreliable by the B1-vs-B2 investigation above. Worse, `QueueAwareDispatchPolicy.default_max_in_flight=4` (which `full` inherits) was itself tuned via a sweep run at that same `agent_replicas=1` condition -- the direct structural parallel to B2's `max_num_seqs=32` bug. Re-ran the ladder (`storm`, B1's frozen config, `causal_only`'s already-proven `--agent-replicas 12 --max-events-per-tick 128`, both systems, 5 reps/policy) with `--dispatch-max-workers 128` raising `naive_concurrent`/`barrier`/`causal_only`/`capability_aware` (generalized from the causal_only-only override, `scripts/run_b1_pilot.py`) -- `sequential` excluded from this run entirely, since its per-tick cost scales linearly with ready-request count (unlike every concurrent rung) and would take many hours at this load; a floor reference, not the subject of the decision gate.

**Result: `full` collapses to a massive, real regression under real load on both systems** -- Roihu -73.2%, LUMI -71.6%, both non-overlapping (`docs/baseline/b1_ladder_realload_{lumi,roihu}_result.json`) -- the exact opposite of the earlier "full recovers, highest throughput of all 7" conclusion.

**Investigating why surfaced a third, previously-unknown confound**, independent of both `max_workers` and `default_max_in_flight`: `BarrierDispatchPolicy`/`CapabilityAwareDispatchPolicy`/`QueueAwareDispatchPolicy`/`FullDispatchPolicy` all default to `BatchBuilder(max_batch_size=8)` (`src/agentic_sim/execution/batcher.py`) -- every backend_hint group is split into sequential chunks of at most 8, one blocking dispatch round per chunk, regardless of `max_workers`. At ~24 ready requests/tick (this run's roster), that forces ~3 sequential rounds, closely matching the observed ~3x gap between `barrier`/`capability_aware` (~2.7-2.8 Roihu, ~0.71-0.72 LUMI) and `causal_only`/`naive_concurrent` (~4.3-8.1 Roihu, ~1.15-1.9 LUMI) in that first run. Added `--dispatch-max-batch-size` (mirrors the existing override pattern exactly, `scripts/run_b1_pilot.py`) and reran with it raised to 128 alongside `--dispatch-max-workers 128`:

| system | policy | first run (batch cap=8) | fixed run (batch cap=128) |
|---|---|---|---|
| LUMI | naive_concurrent | 1.152 | 1.900 |
| LUMI | barrier | 0.723 | 1.992 |
| LUMI | causal_only | 1.886 | 1.944 |
| LUMI | capability_aware | 0.707 | 1.937 |
| LUMI | queue_aware | 0.559 | 0.651 |
| LUMI | full | 0.535 | 0.658 |
| Roihu | naive_concurrent | 4.346 | 7.536 |
| Roihu | barrier | 2.730 | 7.863 |
| Roihu | causal_only | 8.057 | 7.578 |
| Roihu | capability_aware | 2.827 | 7.166 |
| Roihu | queue_aware | 2.127 | 2.664 |
| Roihu | full | 2.158 | 2.743 |

**The batch-size fix fully resolves `naive_concurrent`/`barrier`/`causal_only`/`capability_aware` into a tight cluster on both systems** (1.90-1.99 LUMI, 7.17-7.86 Roihu) -- confirming `max_batch_size=8` was the real, previously-unaddressed cause of `barrier`/`capability_aware`'s apparent underperformance, not a real property of those policies. **But `queue_aware`/`full` remain dramatically, non-overlappingly worse than `causal_only` even with the batch-size confound fixed** -- LUMI -66.1%, Roihu -63.8% -- now cleanly attributable to `default_max_in_flight=4` alone. Zero preemption on either system, either run. This confirms the user's concern precisely, and doubly: the ladder's headline evidence was confounded by two independent, previously-undiscovered mechanisms, and once both are accounted for, `full`'s real standing under genuine load is a large regression, not the earlier-claimed win. Raw data: `docs/baseline/b1_ladder_realload_fixed_{lumi,roihu}_result.json`.

**`default_max_in_flight=4` re-swept under real load -- a real, non-overlapping loser on both systems, exactly mirroring B2's `max_num_seqs` bug.** Swept `{4, 16, 64, 128}` for `queue_aware` alone (`full` inherits, not independently tested here), same `--agent-replicas 12`, `--dispatch-max-batch-size 128` fix applied, `select_best_candidate` reused unmodified (`scripts/run_b2_sweep.py`, exactly the B2 re-sweep's mechanism):

| system | cap=4 | cap=16 | cap=64 | cap=128 | winner |
|---|---|---|---|---|---|
| LUMI | 0.621 | (crashed, excluded -- see below) | 1.938 | 1.890 | **64** (tied with 128) |
| Roihu | 2.782 | 6.449 | 8.388 | 7.636 | **64** (tied with 128) |

Both systems agree: 4 is a real, non-overlapping loser (LUMI's 64 is >3x higher; Roihu's is >3x higher too), 64 and 128 tie, and 64 wins the tie-break as the smaller value -- matching `causal_only`'s own throughput closely on both systems (no longer a regression). LUMI's `cap=16` candidate crashed mid-run (`ValueError: response choice did not include text content`) -- a real, previously-unknown robustness gap in `execution/openai_compatible_backend.py::_first_choice_text`, which raised uncaught outside `run_b1_pilot`'s per-repetition exclusion (`_warm_up` runs before that try/except), crashing the whole pilot rather than excluding one repetition -- the exact same bug class as commit `dbd2a88`'s `storm_env.py` fix, just a different call site. Fixed: `_first_choice_text` now returns `""` instead of raising, funneling into the existing, already-tested invalid-JSON repair path in `_try_parse` (new regression tests in `tests/test_aitta_backend.py`). Zero preemption on either system's sweep. `QueueAwareDispatchPolicy.default_max_in_flight`'s class default corrected from 4 to 64 (`src/agentic_sim/scheduling/dispatch_policy.py`, docstring updated with the full evidence trail); `FullDispatchPolicy` inherits the correction. Raw data: `docs/baseline/b1_ladder_inflight_sweep_{lumi,roihu}_candidates.json`.

**Final confirmatory rerun, both fixes applied (`--dispatch-max-batch-size 128 --dispatch-max-in-flight 64`): the decision gate's answer, definitively.** Same 6-rung ladder, same real load, one more real run per system:

| system | naive_concurrent | barrier | causal_only | capability_aware | queue_aware | full | full_vs_causal_only |
|---|---|---|---|---|---|---|---|
| LUMI | 1.908 | 1.892 | 2.033 | 1.958 | 1.982 | 2.055 | +1.1%, **overlap** |
| Roihu | 7.673 | 7.555 | 7.716 | 7.480 | 7.571 | 7.693 | -0.3%, **overlap** |

**All 6 concurrent rungs are now statistically indistinguishable from each other, on both systems.** Zero preemption, zero excluded repetitions. This is the honest, final answer to the question this whole sub-investigation opened: once genuinely tested under real concurrent load with all three confounds fixed (the original `agent_replicas=1` ceiling, `BatchBuilder.max_batch_size=8`, and `default_max_in_flight=4`), `full` neither beats nor loses to `causal_only` -- it's a **genuine tie**, not the earlier-claimed "highest throughput of all 7" win nor the intermediate "-73%/-72% regression" finding. Per the roadmap's own preregistered decision gate (item 14: "≥15% relative improvement over `causal_only`, non-overlapping, on heterogeneity-bearing variants"): **the gate is not cleared** -- not because `full` is worse, but because there is no measurable difference at all on the one workload tested. Combined with the still-open, structurally deeper caveat that `storm` provides no genuine multi-provider heterogeneity for `capability_aware`/`queue_aware`'s core mechanisms to act on (every real HPC run this session uses exactly one `backend_hint`), this is not yet grounds to invoke the fallback headline either -- it means **the scheduler-led claim has no supporting evidence on the one real-hardware-testable workload available today**, and testing it properly requires a workload with genuine multi-provider heterogeneity, not yet built. Raw data: `docs/baseline/b1_ladder_realload_final_{lumi,roihu}_result.json`. **Still fully open**: standing up genuine multi-provider heterogeneity (two distinct real serving endpoints) to actually test `capability_aware`/`queue_aware`'s core mechanisms -- the single highest-priority gap now, since it's the reason the decision gate remains untestable, not just unresolved; `sequential` was not rerun at this load (cost-prohibitive, excluded by design, though its earlier low-concurrency numbers already show concurrency itself delivers a large, real gain over it); `supply_chain`, full-node placement, and B2 mode remain untested for the ladder; everything else already logged as open above.
