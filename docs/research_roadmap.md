...
The corrected `max_num_seqs` values are notably closer to (LUMI: 64, matching B1 exactly) or larger than (Roihu: 128, exceeding B1's 64) B1's own value -- consistent with the B1-vs-B2 finding above that B2's smaller `max_num_seqs=32` cap was the mechanistic cause of its slowdown under load. This closes the "should B2 be re-tuned using a concurrency-aware methodology" open item raised above: yes, and the correction is now applied -- `docs/b2_frozen_configuration.md` and `docs/baseline/b2_frozen_config_{lumi,roihu}.json` updated accordingly. Raw data: `docs/baseline/b2_concurrent_sweep_{lumi,roihu}_trace.json`. **Still fully open**: whether B2 with this corrected config still trails B1 under load -- the B1-vs-B2 comparison above used the old, now-superseded `max_num_seqs=32`, and has not been rerun against the correction; a higher-repetition confirmatory rerun of the corrected values specifically; `max_num_batched_tokens`'s real-vs-tie-break status remains unresolved under both low and real load; the same `supply_chain` 15-rep confirmation (already non-overlapping at n=5, lower priority); full-node placement, other dispatch policies, or other workloads under real load (none tested here); everything else already logged as open above.
