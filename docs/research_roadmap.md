...
**The batch-size fix fully resolves `naive_concurrent`/`barrier`/`causal_only`/`capability_aware` into a tight cluster on both systems** (1.90-1.99 LUMI, 7.17-7.86 Roihu) -- confirming `max_batch_size=8` was the real, previously-unaddressed cause of `barrier`/`capability_aware`'s apparent underperformance, not a real property of those policies. **But `queue_aware`/`full` remain dramatically, non-overlappingly worse than `causal_only` even with the batch-size confound fixed** -- LUMI -66.1%, Roihu -63.8% -- now cleanly attributable to `default_max_in_flight=4` alone. Zero preemption on either system, either run. This confirms the user's concern precisely, and doubly: the ladder's headline evidence was confounded by two independent, previously-undiscovered mechanisms, and once both are accounted for, `full`'s real standing under genuine load is a large regression, not the earlier-claimed win. Raw data: `docs/baseline/b1_ladder_realload_fixed_{lumi,roihu}_result.json`. **Still fully open, and now the decision-gate-critical next step**: whether `default_max_in_flight=4` itself needs correcting under real load (a sweep, exactly mirroring the B2 `max_num_seqs` correction, is the natural next step and not yet done); the still-unaddressed multi-provider-heterogeneity gap (`capability_aware`/`queue_aware`'s core mechanisms are keyed on `backend_hint`, and every real HPC run this session uses exactly one); `sequential` was not rerun at this load (cost-prohibitive, excluded by design); `supply_chain`, full-node placement, and B2 mode remain untested for the ladder; everything else already logged as open above.
