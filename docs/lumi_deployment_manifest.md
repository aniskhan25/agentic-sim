...
| fixed warm-up, duration, and repetition rule | reuses the existing `AITTA_WARMUP`-style warm-up-then-wait pattern from `scripts/run_lumi.sh`, generalized to poll the self-hosted server's health endpoint instead of Aitta; repetition count and run duration follow `docs/evaluation_plan.md`'s Statistical Design ("a documented warm-up and a minimum repetition count per workload") — the exact count is set when item 18 freezes procedures |
...
Selection procedure: run the suggested benchmark matrix from `amd_vllm_lumi_tuning.md` (tokens/sec, completed agent-steps/sec, TTFT, inter-token latency, GPU utilization, KV-cache preemption count, failed/invalid structured-output rate), select the configuration that maximizes useful agent-steps/sec without KV-cache preemption, and **freeze it before collecting primary results** — per B2's "select configurations through a documented pre-experiment procedure" and "freeze the selected configurations before collecting primary results." Every divergence from the common-denominator configuration above must be recorded (per B2), not just the final chosen values.
