...
Current implementation coverage is narrower, and now correctly labeled and correctly handles the empty case: `message_action_autonomy_rate` aggregates only retained messages and environment actions, reports `None` (unavailable) rather than a fake `1.0` when the committed set is empty, and exposes the explicit zero count as `message_action_committed_atom_count` (`ValidationResult`/`backend_metrics.json`). It deliberately does not yet fold in state-mutation atoms, even though per-atom state-mutation provenance now exists (`ValidationResult.state_mutation_provenance`, added alongside the `bounded`/`cardinality` contracts): messages and environment actions have a real policy-completion counterpart (the guard fills gaps the model left, so kept/total meaningfully contrasts model-authored vs. system-completed), but state mutations do not — `current_goal`/custom `working_memory` keys are either model-set or simply absent, with no "system invents a default" behavior to contrast against. Folding those atoms in today would only push the rate upward whenever a model sets any goal/memory content, without adding real signal. A meaningful full behavior-atom rate needs a genuine "required state mutation" contract type first, which does not exist yet — this stays a documented future step, not built speculatively. Artifacts and results should keep treating `message_action_autonomy_rate` as exactly what its name says, not a substitute for the full behavior-atom metric.
